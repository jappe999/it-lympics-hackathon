...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_scrollmonitor__ = __webpack_require__(58);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_scrollmonitor___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_scrollmonitor__);
...
var app = new Vue({
  ...
  methods: {
    ...
    getFeed: function getFeed() {
      ...
      var url = this.next_url ? '/api/twitter/' + this.next_url : 'api/twitter/all';

      axios.get(url).then(function (response) {
        if (response.data) {
          var search_metadata = response.data.search_metadata;

          if (search_metadata.next_results) {
            _this.next_url = search_metadata.next_results;
          } else {
            var max_id = response.data.statuses[response.data.statuses.length - 1].id - 1;
            _this.next_url = '?q=' + search_metadata.query + '&max_id=' + max_id;
          }
    ...
    parseFeed: function parseFeed(feed) {
      var statuses = feed.statuses;
